...
echo '########## opt_noboth ###########'
python3 bench1.py --timeout $TIMEOUT --benchmark $BENCH_NAME $SCRIPT_NAME
...
echo '########## opt_agg ###########'
python3 bench1.py --timeout $TIMEOUT --benchmark $BENCH_NAME $SCRIPT_NAME --pass-args="--agg"
cp output/0bench_out_append.txt_summary.txt 0bench_out_append.txt_summary_opt_agg.txt
bash ho2.sh file_list/${BENCH_NAME}.txt
cp out1.txt 0bench_out1_opt_agg.txt

echo '########## rd ###########'
python3 bench1.py --timeout $TIMEOUT --benchmark $BENCH_NAME $SCRIPT_NAME --pass-args="--remove-disjunction"
cp output/0bench_out_append.txt_summary.txt 0bench_out_append.txt_summary_opt_rd.txt
bash ho2.sh file_list/${BENCH_NAME}.txt
cp out1.txt 0bench_out1_opt_rd.txt

echo '########## both ###########'
python3 bench1.py --timeout $TIMEOUT --benchmark $BENCH_NAME $SCRIPT_NAME --pass-args="--agg --remove-disjunction"
